...
number_of_dfo_apps = 3 # Should be at least 3 for enable-dfo commands
trigger_rate = 3.0  # Hz
...
required_substring_map = {
    "_mlt": [ "Setting enabled DFO to dfo-02", "Setting enabled DFO to dfo-03" ],
    "_df-\d+": [ "Setting enabled DFO to dfo-02", "Setting enabled DFO to dfo-03" ],
}
...
conf_dict.n_dfo_apps = number_of_dfo_apps
...
nanorc_command_list += (
    "start --run-number 104 wait 5 enable-triggers wait ".split()
    + [str(run_duration)]
    + "disable-triggers wait 1 drain-dataflow wait 2 stop-trigger-sources wait 1 stop wait 2".split()
)
...
def test_log_files(run_nanorc):
    if check_for_logfile_errors:
        ...
        assert log_file_checks.logs_are_error_free(
            run_nanorc.log_files, True, True, ignored_logfile_problems, required_substring_map, True
        )
